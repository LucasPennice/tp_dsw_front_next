  ...
  return (
    <div className="max-w-5xl flex flex-col items-center justify-center gap-4 p-4">
      <p className="text-3xl">
        El servidor de backend tarda aprox 50 segundos en arrancar. El getAll
        esta hecho en este link pero se ve asi
      </p>
      ...
      {profesores.map((profesor) => {
        return (
          <div
            className="bg-gray-900 w-[800px] p-4 rounded-2xl"
            key={profesor.id}
          >
              ...
              <div className="flex justify-between">
                <div className="flex">
                  {Array.from(Array(5).keys()).map((i, idx) => {
                    let color =
                      i < profesor.puntuacionGeneral ? "gold" : "gray";
            ...
            <div className="flex gap-4 flex-wrap pt-4">
              {profesor.cargos.map((cargo, idx) => {
                return (
                  <p
                    className="text-md bg-gray-700 px-4 py-1 rounded-full"
                    key={idx}
                  >
                    {cargo}
                  </p>
      ...
      <div className="col-md-1">
        <p className="">
          LALALALALALALALAALALAL SOY UN COSO DE BOOTSTRAP Lorem ipsum dolor, sit
          amet consectetur adipisicing elit. Nihil non totam earum dolore eius,
          iusto sunt iste nisi suscipit officia velit ullam, sapiente mollitia?
          Quaerat incidunt reprehenderit repellendus aspernatur earum.
        </p>
      </div>
    </div>
  );
